...
from sqlalchemy import create_engine, Insert, Result, Select
from sqlalchemy.orm import sessionmaker
from sqlalchemy.engine import RowMapping
from sqlalchemy.ext.declarative import DeclarativeMeta
from sqlalchemy.dialects.mysql import insert

from data_types import Callable, Iterator
from config.settings import DB_ENDPOINT
...
def yield_chunk_datas(query: Select, chunk_size=3000) -> Iterator:
    ...
    while True:
        with create_session() as session:
            select_stmt = query.offset(offset).limit(chunk_size)  # 가져온 데이터 이후 부터 청크 사이즈 만큼 가져온다.
            offset += chunk_size
